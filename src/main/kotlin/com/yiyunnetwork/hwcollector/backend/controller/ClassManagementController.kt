...
class ClassManagementController {
    ...
    fun getAll(@RequestParam(name = "token") token: String): String {
        ...
        // 查询学生信息
        val student =
            stuRepository.findById(stuName).getOrNull() ?: return Gson().toJson(ClassesInfoData(400, "学生信息不存在！"))
        // 检查学生是否为超级管理员
        if (!student.isAdmin) {
    ...
    fun delete(
        ...
        // 检查班级是否存在
        val classInfo = classInfoRepository.findAll().find { it.className == className } ?: return Gson().toJson(
            SimpleResponseData(
                400,
                "班级不存在！"
            )
